...
(define (negate-bool bool) (if (ask bool 'val)(make-py-bool #f)(make-py-bool #t)))
...
(define-class (py-list val)
  ...
  (method (__contains__ other)
    (define (contain-iter curlist)
      (if (null? curlist)
	  (make-py-bool #f)
	  (if (ask (py-apply (ask (car curlist) '__eq__) (list other)) 'true?)
	      (make-py-bool #t)
	      (contain-iter (cdr curlist)))))
     (contain-iter val))
  (method (append)
