...
(define (py-read)
  ...
  (define (get-num num-so-far)
    ...
    (define (num-iter num-so-far can-see)
      (let ((char (peek-char)))
	(if (char-numeric? char)
	    (num-iter (word num-so-far (char->symbol  (read-char))) can-see)
	    (if(eq? char #\.)
	      (if can-see
		 (num-iter (word num-so-far (char->symbol (read-char))) #f)
		 num-so-far)
		num-so-far)
	    )))
    (num-iter num-so-far #t))
  (define (get-operator)
    (let ((char (read-char))
	  (next (peek-char)))
  ...
  (define (get-string type)
    ...
    (define (string-adder cur-string)
      (if (eq? (peek-char) type)
	  (begin
	    (read-char)
	    cur-string)
	  (string-adder (cons (read-char) cur-string))))
    (reverse (string-adder '()))
    )
  (define (ignore-comment)
    (if(or (eq? (peek-char) #\newline)(eof-object? (peek-char)))
       "Comment-Ignored"
       (begin
	 (read-char)
	 (ignore-comment))))
