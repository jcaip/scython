...
(define (collect-key-value line-obj env close-token)
  (define (get-key cur-list)
    (if(eq?  (ask line-obj 'peek) :)
       (begin
	(ask line-obj 'next)
	(reverse cur-list))
       (get-key (cons (ask line-obj 'next) cur-list))))
  (define (get-val cur-list)
    (if(comma?  (ask line-obj 'peek))
       (begin
	(ask line-obj 'next)
	(reverse cur-list))
       (if(eq? (ask line-obj 'peek) close-token)
	  (begin
	    (display cur-list)
	    (reverse cur-list))
	  (get-val (cons (ask line-obj 'next) cur-list)))))
  (define (create-key-list cur-list)
    (if (eq? (ask line-obj 'peek) close-token)
	(begin
	  (ask line-obj 'next)
	  (reverse cur-list))
	(let ((key-val (py-eval (make-line-obj (cons '*DUMMY-INDENT* (get-key '()))) env)))
	  (create-key-list (cons (cons key-val (py-eval (make-line-obj (cons '*DUMMY-INDENT* (get-val '()))) env))  cur-list)))))
  (create-key-list '()))
...
(define (make-if-block line-obj env)
  (define (get-pred line-obj pred)
    (if (eq? (ask line-obj 'peek) :)
	(begin
	  (ask line-obj 'next)
	  (reverse pred))
	(get-pred line-obj (cons (ask line-obj 'next) pred))))
  (list '*BLOCK* '*IF-BLOCK* (cons '*DUMMY-INDENT* (get-pred line-obj '())) (read-block (ask line-obj 'indentation) env)))
(define (if-block-pred block)
  (caddr block))
(define (if-block-body block)
  (car (split-block (cadddr block))))
(define (if-block-else block)
  (if (pair?  (cdr (split-block (cadddr block))))
      (cdr (split-block (cadddr block)))
      *NONE*))
(define (eval-if-block block env)
 
       (if (ask (py-eval (make-line-obj (if-block-pred block)) env) 'true?)
	   (eval-sequence (if-block-body block) env)
	   (if(pair? (if-block-else block))
...
(define (make-elif-block line-obj env)
  (define (get-pred line-obj pred)
	...
	(begin
	  ...
	  (reverse pred))
	(get-pred line-obj (cons (ask line-obj 'next) pred))))
  (list '*BLOCK* '*ELIF-BLOCK* (cons '*DUMMY-INDENT* (get-pred line-obj '())) (read-block (ask line-obj 'indentation) env)))
(define (elif-block-pred block) (caddr block))
(define (elif-block-body block) (car (split-block (cadddr block))))
(define (elif-block-else block)  (if (pair?  (cdr (split-block (cadddr block))))
      (cdr (split-block (cadddr block)))
      *NONE*))

(define (eval-elif-block block env)
  (eval-if-block (cons '*BLOCK* (cons '*IF-BLOCK* (cddr block))) env))
...
(define (make-while-block line-obj env)
  (define (get-pred line-obj pred)
	...
	(begin
	  ...
	  (reverse pred))
	(get-pred line-obj (cons (ask line-obj 'next) pred))))
  (list '*BLOCK* '*WHILE-BLOCK* (cons (ask line-obj 'indentation)(get-pred line-obj '()))(read-block (ask line-obj 'indentation) env)))
...
(define (make-for-block line-obj env)
  (define (get-collection line-obj pred)
    (if (eq? (ask line-obj 'peek) :)
	(begin
	  (ask line-obj 'next)
	  (reverse pred))
	(get-collection line-obj (cons (ask line-obj 'next) pred))))
  (let ((selector (ask line-obj 'next)))
	(begin (ask line-obj 'next)
	       (list '*BLOCK* '*FOR-BLOCK* selector (cons '*DUMMY-INDENT* (get-collection line-obj '())) (read-block (ask line-obj 'indentation) env)))))
(define (for-block-var block)
  (caddr block))
(define (for-block-collection block)
  (cadddr block))
(define (for-block-body block)
  (car (split-block (list-ref block 4))))
(define (for-block-else block)
  (if (pair? (cdr (split-block (list-ref block 4))))
      (cdr (split-block (list-ref block 4)))
      *NONE*))
  
(define (eval-for-block block env)
        (if (eq? (ask (py-eval (make-line-obj (for-block-collection block)) env) '__iter__ (for-block-var block) (for-block-body block) env) '*BREAK)
	    *NONE*
	    (if(pair? (for-block-else block))
	       (eval-item (make-line-obj (for-block-else block)) env)
	       *NONE*)))
